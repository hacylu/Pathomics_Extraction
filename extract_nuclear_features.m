function [feats,description,feature,display_params] = extract_nuclear_features(bounds,collect_annotation,select)
...
if ~exist('select','var')
    ...
    %     else
    %         %default list
    %         featurelist = [{'Voronoi'} {'Morph'} {'Disorder'} {'Subgraph'} {'Haralick'}  {'ClusterGraph'}];
    %     end
    if iscell(select)
        featurelist = select;
        select= 1:numel(featurelist);
    else
        featurelist = [{'Shape'} {'Voronoi'} {'Delaunay'} {'MinimumSpanningTree'} {'NearestNeighbor'} {'Disorder'} {'Subgraph'} {'Haralick'}  {'ClusterGraph'} {'Collage'}];
    end
end
...
%bounds1 = bounds;
%bounds = remove_bad_nuclei(bounds,img, collect_annotation.MPP);

% img2(:,:,1) = img(:,:,1).*uint8(mask);
% img2(:,:,2) = img(:,:,2).*uint8(mask);
% img2(:,:,3) = img(:,:,3).*uint8(mask);
% figure; imshow(img2)
...
for j = select
    ...
    try
        switch pickfeat
            ...
            case {5, 'Morph','Morphology'}
                
                fprintf('\nExtracting Morph Features...')
                [feats, description] = extract_morph_feats(bounds);
                %    description{2} = swap_feature_header(o.Description.ImageFeatures,'Morph', 'CellMorph');
            ...
            case {7, 'Subgraph'}
                ...
                fprintf('\nExtracting Cell-Graph Features...')
            ...
            case {8, 'Haralick'}
                ...
                if exist('img','var') && exist('mask','var')
                    fprintf('\nExtracting Nuclear Texture Features...')
                    [feats, description, param] = extract_texture_feats(img, mask, collect_annotation.MPP);
            ...
            case {9, 'ClusterGraph'}
                if exist('mask','var')
                    fprintf('\nExtracting Cell-Cluster Graph Features...')
                    %MPP = collect_annotation.MPP;
                    [feats, description,param,display] = extract_cluster_graph_feats(mask,collect_annotation.MPP);
                    %description{4} = swap_feature_header(o.Description.ImageFeatures,'GSG', 'CellGraph');
                end
                
            case {10, 'Collage'}
                if exist('mask','var')
                    fprintf('\nExtracting Nuclear Collage Features...')
                    [feats,description, param] = extract_collage_feats(img,bounds, collect_annotation.MPP);
                end
        end
...
function [morphfeats, description]=extract_morph_feats(bounds)
%% Morph
gb_r = {bounds.r};
gb_c = {bounds.c};
...
function [CCGfeats,description, param,display] = extract_cluster_graph_feats(img,MPP)
...
img_d = imdilate(img, strel('disk', radius));
...
function [Texturefeats,description, param,display] = extract_texture_feats(img,mask, MPP)
...
function [feats,description, param] = extract_collage_feats(img,bounds,MPP)
info.extractionMPP = MPP;

seg_mask = convert_bounds2mask(img,bounds);
%[feats, ~, param]=collage_nuclei_funct(img,seg_mask,info.extractionMPP);
