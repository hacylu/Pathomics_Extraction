 ...
 * haralickims = haralick3mexmt(double volume, double graylevels, double window_size, double dist, double background [optional], double nmaxthreads [optional]) 
 *
 *To compile, it is necessary to link against libut:
 *-WINDOWS (64-bit, visual studio):
 *	mex('-v','-largeArrayDims','haralick3mexmt.cpp',[matlabroot '\extern\lib\win64\microsoft\libut.lib'])
 *-WINDOWS (32-bit, visual studio):
 *  mex('-v','-largeArrayDims','haralick3mexmt.cpp',[matlabroot '\extern\lib\win32\microsoft\libut.lib'])
 *-WINDOWS (32-bit, lcc):
 *  mex('-v','-largeArrayDims','haralick3mexmt.cpp',[matlabroot '\extern\lib\win32\lcc\libut.lib'])
 *-LINUX: mex -v -largeArrayDims haralick3mexmt.cpp -lut
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if(nrhs > 6 || nrhs < 4)
        mexErrMsgTxt("haralick3mexmt(volume,graylevels,ws,dist,[background],[nmaxthreads])");
