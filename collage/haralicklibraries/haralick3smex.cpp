 ...
 * haralickims = haralick3smex(double volume, double graylevels, double window_size, double dist, double background [optional]) 
 *
 *To compile, it is necessary to link against libut:
 *-WINDOWS (64-bit, visual studio):
 *	mex('-v','-largeArrayDims','haralick3smex.cpp',[matlabroot '\extern\lib\win64\microsoft\libut.lib'])
 *-WINDOWS (32-bit, visual studio):
 *  mex('-v','-largeArrayDims','haralick3smex.cpp',[matlabroot '\extern\lib\win32\microsoft\libut.lib'])
 *-WINDOWS (32-bit, lcc):
 *  mex('-v','-largeArrayDims','haralick3smex.cpp',[matlabroot '\extern\lib\win32\lcc\libut.lib'])
 *-LINUX: mex -v -largeArrayDims haralick3smex.cpp -lut
 */
...
void haralick3(float ***imagemat, float *haralicks, int ws, int dist, int graylevels, int background, int rows, int cols, int slices, int nharalicks) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if(nrhs > 5 || nrhs < 4)
        mexErrMsgTxt("haralick3smex(volume,graylevels,ws,dist,background)");
