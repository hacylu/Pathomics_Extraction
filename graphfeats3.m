function [vfeature] = graphfeats2(bin,varargin)
% Necessary input:
% bin: a binary image of points that will be used for graph construction
...
% Graph Generation
% Get the (x,y) coordinates of each nucleus
[x y] = find(bin == 1);
...
% Display Voronoi image (optional)
%if(dispimgflag == 1), figure(1); hold on; h=voronoi(y,x,'sk'); set(h,'LineWidth',1.5,'MarkerSize',2,'MarkerFaceColor','r','MarkerEdgeColor','r'); hold off; end %ANB%
if(dispimgflag == 1), figure(1); hold on; plot(VY,VX,'-k','LineWidth',1.5); plot(y,x,'sk','MarkerSize',5,'MarkerFaceColor','r'); hold off; end %ANB%
if(annotate == 1), title('Voronoi Diagram'); end
...
% ... and plot (optional).
if(dispimgflag == 1), figure(2); hold on; triplot(del,y,x,'k','LineWidth',1.5); plot(y,x,'sk','MarkerSize',5,'MarkerFaceColor','r'); hold off; end %ANB%
if(annotate == 1), title('Delaunay Triangulation'); end

% Get the Minimum Spanning Tree (MST) (optional: plot)
[mst.edges mst.edgelen mst.totlen] = mstree([y,x],[],dispimgflag);
if(annotate == 1), title('Minimum Spanning Tree (MST)'); end
...
for i = 1:length(C)
    ...
    if(~ismember(i,banned))
        ...
        myArea(c) = polyarea(X(:,1),X(:,2));
        c = c + 1;
        clear chord X
    end
end

vfeature(1) = std(myArea); 
vfeature(2) = mean(myArea);
vfeature(3) = min(myArea) / max(myArea);
...
kcount = 1;
for K = [3,5,7] %%%%%% ANB %%%%% REMOVED DUE TO LACK OF NN
...
vfeature(34) = 1 - (1 / ( 1 + (vfeature(31) / vfeature(28)) ));
vfeature(35) = 1 - (1 / ( 1 + (vfeature(32) / vfeature(29)) ));
vfeature(36) = 1 - (1 / ( 1 + (vfeature(33) / vfeature(30)) ));
...
function [edges,edgelen,totlen] = mstree(crds,labels,doplot)
  ...
  if (doplot)
    figure(3); hold on; %ANB%
    plot(crds(:,1),crds(:,2),'sk','MarkerFaceColor','r','MarkerSize',5);
